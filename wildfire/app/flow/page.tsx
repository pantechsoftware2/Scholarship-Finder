...
export default function FlowPage() {
  ...
  useEffect(() => {
    ...
    const id = setInterval(
      () => setLoaderIndex((prev) => (prev + 1) % LOADER_LINES.length),
      1200
    );
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // Store reportId in sessionStorage for tab navigation
  ...
  function togglePower(p: SpecialPower) {
    ...
    setForm((prev) => ({
      ...prev,
      specialPowers: prev.specialPowers.includes(p)
        ? prev.specialPowers.filter((x) => x !== p)
        : [...prev.specialPowers.filter((x) => x !== "None"), p],
    }));
  }
  ...
  return (
    <main className="relative min-h-screen bg-[radial-gradient(circle_at_top,_#020617,_#000000)] text-white flex items-center justify-center px-4">
      ...
      {/* alive radar loader using your CSS classes */}
      <AnimatePresence>
        {loading && (
          <motion.div
            initial={{ opacity: 0 }}
